$(function() {
    ...
    // При наличии данных в cookie переписать список.
        ...
        $list.empty();
        list.forEach(item => {
            appendItem(item.name, item.date, item.selected);
        });
    }
    ...
    function appendItem(name, date, selected) {
        $list.append(`
            <li class="ui-sortable-handle ${selected ? 'ui-selected' : ''}">
                <p>
    ...
    function addItem() {
        ...
        if (text.length !== 0) {
            ...
            let date = new Date();
            let textDate = (`${addZero(date.getDate())}.${addZero(
                date.getMonth() + 1)}.${date.getFullYear()} ${date.getHours()}:${addZero(date.getMinutes())}`);
    ...
    function updateSelectedCounter() {
        ...
        $('#selected-counter').text(`Выбрано: ${selected} ${declination(selected, 'дело', 'дела', 'дел')}`);
    }
    ...
    $list.selectable({
        stop: function() {
            $list.find('li').each(function(i) {
                list[i].selected = $(this).hasClass('ui-selected');
            });
            updateSelectedCounter();
            saveList();
        }
